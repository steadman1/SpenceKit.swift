...
public extension View {
    ...
    func textStroke(color: Color, width: CGFloat = SpenceKit.Constants.borderWidth) -> some View {
        modifier(TextStrokeModifier(strokeSize: width / 2, strokeColor: color))
    }
            ...
            .background(
                ZStack {
                    ForEach(0..<8) { index in
                        let angle = Double(index) * (Double.pi / 4) // 45 degrees * index
                        let x = strokeSize * cos(angle)
                        let y = strokeSize * sin(angle)
